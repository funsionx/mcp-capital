/**
 * Russian Trusted Root CA + Sub CA (Минцифры) — the trust anchors for TBank's API
 * cert chain (`*.tbank.ru` -> Russian Trusted Sub CA -> Russian Trusted Root CA).
 * This root is not in default trust stores, so we pin it and pass it as an
 * *additional* trust anchor; TLS verification stays ENABLED (no MITM exposure).
 *
 * These are PUBLIC certificates (not secrets). Inlined so there's no runtime file
 * dependency.
 *
 * Root cert validity: 2022-03-01 .. 2032-02-27
 * Root SHA-256: D2:6D:2D:02:31:B7:C3:9F:92:CC:73:85:12:BA:54:10:35:19:E4:40:5D:68:B5:BD:70:3E:97:88:CA:8E:CF:31
 */
export const TBANK_CA = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`;
